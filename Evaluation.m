...
prediction_path = '../predictions_unaug/';
...
%% Evaluate deep learning model with augmentation
prediction_path = '../predictions_aug/';
pred_folder= dir(prediction_path);
pred_file={pred_folder.name};
dice_coef_aug = zeros(1, length(pred_file)-3);
case_name_list = string(pred_file(4:length(pred_file)));
for num_pred= 4 : length(pred_file)
    case_name = pred_file(num_pred);
    case_name = char(case_name);
    ...
    pred = logical(mat.pred);   
    dice =  2*nnz(mask&pred)/(nnz(mask) + nnz(pred));
    dice_coef_aug(num_pred-3) = dice;

end
    dice_aug_avg = mean(dice_coef_aug)
...
%% plot
figure(1)
x = cat(2, dice_coef_dl', dice_coef_aug');
...
line([dice_aug_avg, dice_aug_avg], ylim, 'Color','red','LineStyle','--', 'LineWidth',2 );
%line([dice_bse_avg, dice_bse_avg], ylim, 'Color','red','LineStyle','--', 'LineWidth',2 );
text(dice_dl_avg, ylim(2), num2str(dice_dl_avg), 'color','b');
text(dice_aug_avg, ylim(2), num2str(dice_aug_avg), 'color','r');
%text(dice_bse_avg, ylim(2), num2str(dice_bse_avg), 'color','r');
%legend({'U-Net', 'Aug-U-Net', 'Brainsuite', 'Avg DSC for UNet', 'Avg DSC for Aug-UNet','Avg DSC for brainsuite'});
legend({'U-Net', 'Aug-U-Net', 'Avg DSC for UNet', 'Avg DSC for Aug-UNet'});
